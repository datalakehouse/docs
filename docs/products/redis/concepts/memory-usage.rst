Memory usage, on-disk persistence and replication in DLH for Redis
=====================================================================

Here we show you how DLH for Redis solves the challenges related to high memory usage and high change rate. 
...
Data eviction policy in DLH for Redis
---------------------------------------

Data eviction policy is one of the most important Redis settings in this case, it is available in the DLH web console. 

Redis has a ``max memory`` setting which controls how much data is allowed to be stored, and the data eviction policy controls what happens when that maximum is reached. All DLH for Redis services have the eviction policy set to *No eviction* by default. This means that if you keep storing values, and never remove anything, the write operations will start failing when the maximum memory is reached.
...
For all new DLH for Redis services the ``max memory`` is set to **70% of available RAM** (minus management overhead) plus 10% for replication log. The memory usage is limited to below 100% because of the following situations when Redis performs operations that can require additional memory: 
...
- Similar forking is done when Redis persists its current state on disk. Currently, for an DLH for Redis service this happens **every 10 minutes**.
...
The rate at which data can be written depends on the size of values that are being written, and the DLH plan details.  
...
DLH does not have a rate limit traffic for Redis services because limiting only the relevant write operations requires specialized Redis proxy, and restricting all traffic can affect non-write traffic. 
...
.. Note:: 
    If you have a use case where you need to constantly write large amounts of data you can contact DLH support to discuss options for your service configuration.