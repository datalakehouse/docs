...
For current DLH for Elasticsearch customers, we recommend you upgrade as soon as possible so that we can continue to support your database requirements. Depending on your needs, the various options for testing and upgrading the service are detailed here. The upgrade does not require downtime, and any Kibana service alongside Elasticsearch will be upgraded to OpenSearch Dashboards at the same time.
...
* Create a :doc:`fork </docs/platform/concepts/database-forking>` of DLH for OpenSearch from your existing DLH for Elasticsearch service. This is a good way to get a fast preview of how DLH for OpenSearch will work for you.
* Create a fork of DLH for Elasticsearch from your existing DLH for Elasticsearch service. This gives you the opportunity to test the process of upgrading an existing Elasticsearch service to OpenSearch without experimenting on your live database.
* Upgrade from DLH for Elasticsearch to DLH for OpenSearch to move either a test or production database to the upgraded state.
...
2. When forking DLH for Elasticsearch services, there in an additional option to specify the **Service type** as either Elasticsearch or OpenSearch. Choose OpenSearch and give your new service a name.
...
When the operation completes, your DLH for OpenSearch service should continue to operate much as your Elasticsearch service did.
