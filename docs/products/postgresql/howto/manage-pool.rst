...
You can connect directly to the PostgreSQL server using the **Service URI** setting listed on the **Overview** page however this URI doesn't make use of PgBouncer pooling. PgBouncer pools use a different port number than the regular PostgreSQL server port, the PgBouncer **Service URI** for a particular pool can be found in the DLH console, under the **Pool** tab in the service page.
...
For example, if the primary connection URI is::
    ...
    postgres://avnadmin:password@pg-prod-myproject.DLHcloud.com:20986/mypool?params

The replica connection pool URI is::

    postgres://avnadmin:password@replica-pg-prod-myproject.DLHcloud.com:20986/mypool?params
