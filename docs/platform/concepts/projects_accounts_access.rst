...
The datalakehouse platform uses accounts and projects to organize services and access to those services. This article describes those terms and how you can use them effectively based on your organization structure, deployment environments, and service security requirements.

There are two ways that you can manage access to datalakehouse services:
...
You can use accounts and teams within the datalakehouse platform to implement Security Assertion Markup Language single sign-on (SAML SSO) using an identity provider such as Okta, GSuite, or AzureAD. Security-conscious teams usually favor a combination of SAML and RBAC regardless of the size of team.
...
Each project must have a unique name within the datalakehouse platform. The project name is combined with the service names to create the host URLs for your deployed services.


Accounts
--------

An account is a collection of projects. When you first sign up to datalakehouse, there are no accounts, as you can use standalone projects without ever needing accounts.

If you have several different departments that are using datalakehouse, you can use accounts to separate access between projects and departments.
...
You can invite people to work with you on a project, but you may not always want to give them the same access that you have. You can specify the email addresses and permissions for members in the *Members* section of the datalakehouse web console, which you can find in the main menu on the left of the page.

The roles and corresponding permissions that datalakehouse supports are:
